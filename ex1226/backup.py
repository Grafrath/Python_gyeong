...
# 음악 관련 기능
def play_game_music():
    """10% 확률로 히든 음악을 재생하는 함수"""
    is_hidden = random.randint(1, 10) == 1
    bgm_file = HIDDEN_BGM if is_hidden else NORMAL_BGM
    full_path = osㅅ.path.join(MUSIC_DIR, bgm_file)
    try:
        pygame.mixer.music.load(full_path)
        pygame.mixer.music.set_volume(0.4)
        pygame.mixer.music.play(-1)
    except:
        print(f"음악 파일을 로드할 수 없습니다.")

def load_sounds():
    """모든 효과음을 로드하여 딕셔너리로 반환"""
    sounds = {}
    try:
        ...
        # 볼륨 설정
        sounds['lvl_up'].set_volume(0.4)
        sounds['clear'].set_volume(0.6)
        sounds['drop'].set_volume(0.4)
    except:
        print("일부 효과음 파일을 찾을 수 없습니다.")
    return sounds

def play_ending_music(level, score):
    """조건별 엔딩 음악 재생 및 블루스크린 처리"""
    pygame.mixer.music.stop()
    
    # 5,000점 미만: 데스매치 블루스크린 연출
    if score < 5000:
        try:
    ...
    # 레벨별 엔딩 곡 선정
    if level <= 5: file = SND_ENDING_LV5
    elif level <= 10: file = SND_ENDING_LV10
    else: file = SND_ENDING_TRUE
...
def draw_bsod(screen, score):
    ...
    font_lg = pygame.font.SysFont(font_name, 120, bold=True)
    font_md = pygame.font.SysFont(font_name, 40, bold=True)
    font_sm = pygame.font.SysFont(font_name, 25)
    
    msg1 = font_lg.render(":(", True, WHITE)
    msg2 = font_md.render("재능 부족 에러가 발생했습니다.", True, WHITE)
    msg3 = font_sm.render(f"ERROR_CODE: TALENT_NOT_FOUND (Score: {score}/5000)", True, WHITE)
    msg4 = font_sm.render("당신의 실력으로는 시스템을 더 이상 가동할 수 없습니다.", True, WHITE)
    msg5 = font_sm.render("처음부터 다시 배우고 오세요. 게임을 강제 종료합니다.", True, WHITE)
    
    center_x = info.current_w // 10
    screen.blit(msg1, (center_x, info.current_h // 5))
    screen.blit(msg2, (center_x, info.current_h // 5 + 200))
    screen.blit(msg3, (center_x, info.current_h // 5 + 300))
    screen.blit(msg4, (center_x, info.current_h // 5 + 350))
    screen.blit(msg5, (center_x, info.current_h // 5 + 550))
...
def render_all(board, cp, n_queue, hp, score, level, combo, is_game_over, is_paused, particles, flash_alpha, shake_offset, lv_up_timer):
    ...
    if hp:
        ...
        bx, by = sidebar_x + (150 - len(shape[0])*p_size)//2, 310 + (80 - len(shape)*p_size)//2
        for sy, srow in enumerate(shape):
            for sx, scell in enumerate(srow):
                if scell: draw_block(temp_surf, bx+sx*p_size, by+sy*p_size, color, size=p_size)
...
def main():
    ...
    pygame.display.set_caption("Tetris: Deathmatch Edition")
                ...
                if lock_timer > current_lock_delay:
                    ...
                    if full_l:
                        ...
                        if (total_l // 15) + 1 > level:
                            level += 1; lv_up_timer, shake_intensity = 90, 20
                        ...
                        if any(cell and cp["y"]+y < 0 for y, row in enumerate(cp["shape"]) for x, cell in enumerate(row)):
                            is_game_over = True
                            ending_type = play_ending_music(level, score)
                            if ending_type == "BSOD": draw_bsod(screen, score)
                    
                    if not is_game_over:
                        cp, n_queue, can_hold = n_queue.pop(0), n_queue + [get_piece()], True
                        if not is_valid_move(board, cp, adj_y=1):
                            is_game_over = True
                            ending_type = play_ending_music(level, score)
                            if ending_type == "BSOD": draw_bsod(screen, score)
                        lock_timer = 0
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if not is_game_over and not is_paused:
                    if (event.key in [pygame.K_LSHIFT, pygame.K_c]) and can_hold:
                        old_name = cp["name"]
                        if hp is None: cp, hp = n_queue.pop(0), old_name; n_queue.append(get_piece())
                        else: cp, hp = get_piece(hp), old_name
                        can_hold = False
